...
| Polymarket cancel (execute) | \`byreal-cli polymarket order cancel --order-id <id> --execute\` |
| Polymarket account deploy | \`byreal-cli polymarket account deploy --execute\` |
| Polymarket deposit (execute) | \`byreal-cli polymarket funding deposit --amount <usdc> --execute\` |
| Polymarket deposit preview | \`byreal-cli polymarket funding deposit --amount <usdc> --dry-run\` |
| Polymarket withdraw preview | \`byreal-cli polymarket funding withdraw --amount <usdc> --dry-run\` |
| Polymarket withdraw (execute) | \`byreal-cli polymarket funding withdraw --amount <usdc> --execute\` |
...
- For selling from portfolio, \`positions[].position_id\` is the **outcome token id** (same as Polymarket data-api \`asset\`, CLOB \`asset_id\`, and order \`--token-id\`). Match the user's requested holding by \`market_title\`/\`outcome_label\`, then pass \`--token-id <position_id>\`, \`--condition-id <condition_id>\`, and \`--size <sellable_size or chosen shares>\`.
...
- Use \`funding deposit --dry-run\` / \`funding withdraw --dry-run\` for read-only funding previews (quote + min, from \`bridge/supported-assets\`); P0 supports **Solana USDC ⇄ Polygon** only. \`funding status\` reads \`/bridge/orders\` (COMPLETED/FAILED terminal). All resolve the proxy wallet first, so they need a deployed proxy. **Deposit** source is the **embedded Solana wallet** in realclaw-config (only it can be signed by the agent token), NOT your main/Phantom wallet. **Withdraw** always returns funds to that same embedded Solana wallet; CLI intentionally does not accept arbitrary recipients.
...
byreal-cli polymarket order preview     --token-id <yesTokenId> --side buy --amount 1 -o json
byreal-cli polymarket account readiness --token-id <yesTokenId> --side buy --amount 1 --condition-id <cond> -o json
byreal-cli polymarket order place        --token-id <yesTokenId> --side buy --amount 1 --dry-run -o json
byreal-cli polymarket order place        --token-id <yesTokenId> --side buy --amount 1 --execute -o json
\`\`\`
...
- **Limit** orders (\`--order-type limit\`) are **GTC** (resting): \`encode(price) → sign → submit → keepalive\`. Both sides use \`--price\` (0<p<1) + \`--size\` (shares); no book sweep, no \`--preview\`. On HTTP 200 the order is \`outcome: "accepted"\` (resting) and the backend keepalive is registered (best-effort; a keepalive failure does not fail the order). CLOB order-list reads are eventually consistent: after submit, \`order active\` may briefly omit the new order; after cancel, it may briefly still show the canceled order. Do **not** infer success/failure from one immediate \`order active\` read—check \`order status --order-id <id>\` and retry \`order active\` shortly. ⚠️ A resting order can still be auto-canceled by the CLOB if heartbeats lapse — re-check later with \`order active\`/\`order status\`.
- Limit orders below the book \`min_order_size\` are rejected by CLOB (verified for SELL too: \`Size (1) lower than the minimum: 5\`). Keep the local limit-order minimum check. This is separate from market SELL, where small partial sells are allowed and should not be locally gated by \`min_order_size\`.
...
- **Withdraw:** \`funding withdraw --amount <usdc> --execute\` bridges Polygon proxy pUSD → the embedded Solana USDC wallet from realclaw-config. It is **CLI zero-signing** — the backend encodes, signs via Privy, and relays; the CLI only POSTs \`{walletAddress=proxy, toChainId, toTokenAddress, recipientAddr=<configured Solana wallet>, amount, quoteId}\`. Poll ≤30s → pending; re-check via \`funding status --type withdraw\`. ⚠️ The backend signs withdraw typed-data under a dedicated Privy policy (\`bridge_withdraw_v1\`); until the agent token is granted that policy the gateway returns \`40902\` (a backend/policy gap, not a CLI bug) — the CLI surfaces a clear hint.
