...
const CAPABILITIES: Capability[] = [
  ...
  },
  {
    ...
    description: 'Best-of-three aggregator routing — fans out quotes to Jupiter, Titan, and DFlow in parallel, picks the highest output, and returns the unsigned transaction. Use when both tokens are NOT on a Byreal CLMM pool. Requires --wallet-address global option.',
    category: 'execute',
    auth_required: true,
    command: 'byreal-cli aggregator swap --wallet-address <address>',
    params: [
      { name: 'input-mint', type: 'string', required: true, description: 'Input token mint address' },
      { name: 'output-mint', type: 'string', required: true, description: 'Output token mint address' },
      { name: 'amount', type: 'string', required: true, description: 'Amount to swap (UI format, decimals auto-resolved)' },
      { name: 'slippage', type: 'integer', required: false, description: 'Slippage tolerance in basis points' },
      { name: 'max-price-impact-pct', type: 'number', required: false, description: 'Hard-reject quotes with priceImpactPct above this (Titan exempt — its RFQ schema omits the field)', default: '1.0' },
      { name: 'prefer', type: 'string', required: false, description: 'Skip comparison, force one aggregator (still subject to price-impact gate)', enum: ['jup', 'titan', 'dflow'] },
      { name: 'include', type: 'string', required: false, description: 'Comma-separated subset of aggregators to query', default: 'jup,titan,dflow' },
      { name: 'quote-timeout-ms', type: 'integer', required: false, description: 'Per-aggregator quote timeout in ms', default: '8000' },
      { name: 'raw', type: 'boolean', required: false, description: 'Amount is already in raw (smallest unit) format' },
      { name: 'dry-run', type: 'boolean', required: false, description: 'Compare quotes only; do not generate the unsigned transaction' },
    ],
  },
  {
  ...
  },
  {
    ...
    name: 'Open Position',
    description: 'Open a new CLMM position. Two modes: --amount (token amount) or --amount-usd (USD budget, auto-splits into tokenA/B). --dry-run checks wallet balance and reports deficit with swap suggestions. Requires --wallet-address global option.',
    category: 'execute',
    auth_required: true,
  ...
  },
  {
    ...
    description: 'Add liquidity to an existing position. Two modes: --amount (token amount with --base) or --amount-usd (USD budget, auto-splits). --dry-run checks wallet balance. Requires --wallet-address global option.',
    category: 'execute',
    auth_required: true,
  ...
  },
  {
    ...
    description: 'Partially remove liquidity from a position (keeps position NFT open, unlike close which burns it). Two modes: --percentage (by ratio) or --amount-usd (by USD value). Requires --wallet-address global option.',
    category: 'execute',
    auth_required: true,
  ...
  },
  {
    ...
    name: 'Close Position',
    description: 'Close a position (remove all liquidity and burn position NFT). Requires --wallet-address global option.',
    category: 'execute',
    auth_required: true,
