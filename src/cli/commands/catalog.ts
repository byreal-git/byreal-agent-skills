...
const CAPABILITIES: Capability[] = [
  ...
  },
  {
    id: 'wallet.balance',
    name: 'Wallet Balance',
    description: 'Query SOL and SPL token balances for a given wallet address',
    category: 'query',
    auth_required: false,
  ...
  },
  {
    id: 'dex.swap.execute',
    name: 'Swap Execute',
    description: 'Preview (--dry-run) or execute a token swap. Requires --wallet-address global option.',
    category: 'execute',
    auth_required: true,
    command: 'byreal-cli swap execute --wallet-address <address>',
    params: [
  ...
  },
  {
    ...
    name: 'Open Position',
    description: 'Open a new CLMM position. Two modes: --amount (token amount) or --amount-usd (USD budget, auto-splits into tokenA/B). Add --auto-swap to do Zap-In from a single token (requires --execute). --dry-run checks wallet balance and reports deficit with swap suggestions. Requires --wallet-address global option.',
    category: 'execute',
    auth_required: true,
    command: 'byreal-cli positions open --wallet-address <address>',
    params: [
      ...
      { name: 'base', type: 'string', required: false, description: 'Base token (required with --amount or --auto-swap). Accepts MintA, MintB, or a raw mint address with --auto-swap.', enum: ['MintA', 'MintB'] },
      { name: 'amount', type: 'string', required: false, description: 'Amount of base token (UI format). Mutually exclusive with --amount-usd.' },
      { name: 'amount-usd', type: 'string', required: false, description: 'Investment in USD. Auto-calculates token A/B split. Mutually exclusive with --amount and --auto-swap.' },
      { name: 'slippage', type: 'integer', required: false, description: 'Slippage tolerance in basis points' },
      { name: 'raw', type: 'boolean', required: false, description: 'Amount is already in raw (smallest unit) format' },
      { name: 'auto-swap', type: 'boolean', required: false, description: 'Zap-In via Byreal router: convert single-token --base+--amount into the position split. Requires --execute (or --dry-run for preview); incompatible with --amount-usd and with default unsigned-tx mode.' },
      { name: 'dry-run', type: 'boolean', required: false, description: 'Preview the position without opening' },
      { name: 'execute', type: 'boolean', required: false, description: 'Sign + broadcast via Privy proxy (required for --auto-swap)' },
    ],
  },
  {
    id: 'dex.position.increase',
    name: 'Increase Liquidity',
    description: 'Add liquidity to an existing position. Two modes: --amount (token amount with --base) or --amount-usd (USD budget, auto-splits). Add --auto-swap to do Zap-In from a single token (requires --execute). --dry-run checks wallet balance. Requires --wallet-address global option.',
    category: 'execute',
    auth_required: true,
    command: 'byreal-cli positions increase --wallet-address <address>',
    params: [
      { name: 'nft-mint', type: 'string', required: true, description: 'Position NFT mint address' },
  ...
  },
  {
    ...
    description: 'Partially remove liquidity from a position (keeps position NFT open, unlike close which burns it). Two modes: --percentage (by ratio) or --amount-usd (by USD value). Add --auto-swap to do Zap-Out into a single token (requires --execute + --output-mint + --percentage). Requires --wallet-address global option.',
    category: 'execute',
    auth_required: true,
    command: 'byreal-cli positions decrease --wallet-address <address>',
    params: [
      { name: 'nft-mint', type: 'string', required: true, description: 'Position NFT mint address' },
      { name: 'percentage', type: 'string', required: false, description: 'Percentage of liquidity to remove (1-100). Mutually exclusive with --amount-usd. Required when --auto-swap is used.' },
      { name: 'amount-usd', type: 'string', required: false, description: 'USD amount of liquidity to remove. Auto-calculates percentage. Mutually exclusive with --percentage and --auto-swap.' },
      { name: 'slippage', type: 'integer', required: false, description: 'Slippage tolerance in basis points' },
      { name: 'auto-swap', type: 'boolean', required: false, description: 'Zap-Out via Byreal router: convert withdrawn dual-token output into a single --output-mint. Requires --execute (or --dry-run for preview), --output-mint, --percentage; incompatible with --amount-usd and default unsigned-tx mode.' },
      { name: 'output-mint', type: 'string', required: false, description: 'Target mint for --auto-swap output (must be one of the pool\'s two mints).' },
      { name: 'dry-run', type: 'boolean', required: false, description: 'Preview without executing' },
      { name: 'execute', type: 'boolean', required: false, description: 'Sign + broadcast via Privy proxy (required for --auto-swap)' },
    ],
  },
  {
    ...
    name: 'Close Position',
    description: 'Close a position (remove all liquidity and burn position NFT). Add --auto-swap to do Zap-Out into a single token plus best-effort incentive preclaim before close (requires --execute + --output-mint). Requires --wallet-address global option.',
    category: 'execute',
    auth_required: true,
    command: 'byreal-cli positions close --wallet-address <address>',
    params: [
      { name: 'nft-mint', type: 'string', required: true, description: 'Position NFT mint address' },
      { name: 'slippage', type: 'integer', required: false, description: 'Slippage tolerance in basis points' },
      { name: 'auto-swap', type: 'boolean', required: false, description: 'Zap-Out via Byreal router: convert withdrawn dual-token output into a single --output-mint and preclaim pending incentives. Requires --execute (or --dry-run for preview) and --output-mint; incompatible with default unsigned-tx mode.' },
      { name: 'output-mint', type: 'string', required: false, description: 'Target mint for --auto-swap output (must be one of the pool\'s two mints).' },
  ...
  },
  {
    ...
    name: 'Claim Fees',
    description: 'Claim accumulated fees from one or more positions. Uses NFT mint addresses (same as positions list and close). Requires --wallet-address global option.',
    category: 'execute',
    auth_required: true,
    command: 'byreal-cli positions claim --wallet-address <address>',
    params: [
  ...
  },
  {
    ...
    name: 'Claim Rewards',
    description: 'Claim incentive rewards from positions (operational rewards added to pools). --dry-run previews unclaimed amounts per position. Requires --wallet-address global option.',
    category: 'execute',
    auth_required: true,
    command: 'byreal-cli positions claim-rewards --wallet-address <address>',
    params: [
  ...
  },
  {
    ...
    name: 'Claim Bonus',
    description: 'Claim CopyFarmer bonus rewards (earned from copying positions). --dry-run shows epoch bonus overview and claimable amount. Requires --wallet-address global option.',
    category: 'execute',
    auth_required: true,
    command: 'byreal-cli positions claim-bonus --wallet-address <address>',
    params: [
  ...
  },
  {
    ...
    name: 'Copy Position',
    description: 'Copy an existing position with the same price range. Records a referral on-chain for copy bonus rewards (yield boost + referral fees). Requires --wallet-address global option.',
    category: 'execute',
    auth_required: true,
    command: 'byreal-cli positions copy --wallet-address <address>',
    params: [
