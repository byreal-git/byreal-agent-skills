 ...
 *    (encodeReward type=1 -> privySignMany -> submitRewardOrder); on failure
 *    return best-effort and let the main flow continue.
 *
 * openclaw branch: signing is delegated to the Privy proxy via `privySignMany`.
 * A Privy failure surfaces with the proxy's error message and the main flow
 * decides whether to abort or continue.
 */
import chalk from 'chalk';

import { api } from '../../api/endpoints.js';
import { privySignMany, type PrivyContext } from '../../privy/index.js';
 ...
 * @param privyCtx         Privy proxy context (token + endpoint + wallet id).
...
export async function runIncentivePreclaim(
  ...
  // 3) Sign all preclaim txs via Privy (sign-only batch).
  ...
  if (!signResult.ok) {
    if (verbose) {
      console.log(
        chalk.yellow(
          `  [preclaim] Privy sign failed: ${signResult.error.message}. Continuing with close (best-effort).`,
        ),
      );
    }
    return {
  ...
  const signedPayloads = rewardEncodeItems.map((item, i) => ({
    txCode: item.txCode,
