...
const capabilities: Capability[] = [
  ...
  },
  {
    ...
    description:
      'Place a market (FOK; re-quote → Privy sign → submit → poll) or limit (GTC; user price → submit → keepalive) order',
    category: 'execute',
    auth_required: true,
    ...
    params: [
      ...
      { name: 'size', type: 'string', required: false, description: 'SELL market, or limit (both sides): shares' },
      { name: 'price', type: 'string', required: false, description: 'Limit price (limit orders only; 0 < p < 1)' },
      { name: 'condition-id', type: 'string', required: false, description: 'Market conditionId (readiness market checks)' },
      { name: 'slippage-bps', type: 'integer', required: false, description: 'Absolute Δ market slippage (default 100=0.01)' },
      { name: 'preview', type: 'string', required: false, description: 'Preview snapshot JSON for market PREVIEW_EXPIRED check' },
      { name: 'evm-wallet-address', type: 'string', required: false, description: 'EVM EOA (defaults to realclaw-config evm wallet)' },
  ...
  {
    id: 'pm.order.active',
    name: 'Polymarket Active Orders',
    description: 'List active (open) orders (L2)',
    category: 'query',
    auth_required: true,
    command: 'byreal-cli polymarket order active',
    params: [
      { name: 'market', type: 'string', required: false, description: 'Filter by market conditionId' },
      { name: 'asset-id', type: 'string', required: false, description: 'Filter by outcome token id' },
      { name: 'evm-wallet-address', type: 'string', required: false, description: 'EVM EOA' },
    ],
  },
  {
    id: 'pm.order.status',
    name: 'Polymarket Order Status',
    description: 'Read a single order status (L2)',
    category: 'query',
    auth_required: true,
    command: 'byreal-cli polymarket order status --order-id <id>',
    params: [
      { name: 'order-id', type: 'string', required: true, description: 'CLOB order id' },
      { name: 'evm-wallet-address', type: 'string', required: false, description: 'EVM EOA' },
    ],
  },
  {
    id: 'pm.order.cancel',
    name: 'Polymarket Order Cancel',
    description: 'Cancel open orders (--dry-run previews the target set; --execute cancels)',
    category: 'execute',
    auth_required: true,
    command: 'byreal-cli polymarket order cancel --order-id <id> --execute',
    params: [
      { name: 'order-id', type: 'string', required: false, description: 'Cancel a specific order id' },
      { name: 'all', type: 'boolean', required: false, description: 'Cancel ALL open orders' },
      { name: 'market', type: 'string', required: false, description: 'Filter target set by market' },
      { name: 'asset-id', type: 'string', required: false, description: 'Filter target set by token id' },
      { name: 'evm-wallet-address', type: 'string', required: false, description: 'EVM EOA' },
      { name: 'execute', type: 'boolean', required: false, description: 'Cancel (write); default previews' },
      { name: 'dry-run', type: 'boolean', required: false, description: 'Preview target set only' },
    ],
  },
  {
