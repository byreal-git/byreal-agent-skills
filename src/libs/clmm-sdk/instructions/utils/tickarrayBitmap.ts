...
export class TickArrayBitmapExtensionUtils {
  ...
  public static checkTickArrayIsInit(
    ...
    const { tickarrayBitmap } = this.getBitmap(
      tickArrayStartIndex,
      tickSpacing,
  ...
  public static nextInitializedTickArrayInBitmap(
    ...
    const tickArrayOffsetInBitmap = this.tickArrayOffsetInBitmap(
      nextTickArrayStartIndex,
      tickSpacing
    );
    if (zeroForOne) {
      ...
      if (nextBit !== null) {
        const nextArrayStartIndex =
          nextTickArrayStartIndex - nextBit * TickQuery.tickCount(tickSpacing);
        return { isInit: true, tickIndex: nextArrayStartIndex };
      } else {
      ...
      const offsetBitMap =
        TickUtils.mergeTickArrayBitmap(tickarrayBitmap).shrn(tickArrayOffsetInBitmap);
      ...
      if (nextBit !== null) {
        ...
        return {
          isInit: false,
          tickIndex: bitmapMaxTickBoundary - TickQuery.tickCount(tickSpacing),
        };
      }
    }
  }
